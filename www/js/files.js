...
function init_files_panel(dorefresh) {
	...
	id("files_createdir_btn").addEventListener("click", (event) =>
		files_Createdir(),
	);
	id("files_filter_btn").addEventListener("click", (event) =>
		files_filter_button(),
	);

	id("files_refresh_btn").addEventListener("click", (event) =>
		files_refreshFiles(files_currentPath),
	);
	id("files_refresh_primary_sd_btn").addEventListener("click", (event) =>
		files_refreshFiles(primary_sd),
	);
	id("files_refresh_secondary_sd_btn").addEventListener("click", (event) =>
		files_refreshFiles(secondary_sd),
	);
	...
	id("print_upload_btn").addEventListener("click", (event) =>
		files_select_upload(),
	);

	id("files_input_file").addEventListener("change", (event) =>
		files_check_if_upload(),
	);
...
const files_set_button_as_filter = (isfilter) =>
	setHTML(
		"files_filter_glyph",
		get_icon_svg(!isfilter ? "filter" : "list-alt", "1em", "1em"),
	);
...
function files_build_file_line(index, actions) {
	...
	if ((files_filter_sd_list && entry.isprintable) || !files_filter_sd_list) {
		...
		if (!entry.isdir) {
			...
			actions.push({
				id: `${fliId}_download_btn`,
				type: "click",
				method: files_download(index),
			});
		}
		if (files_showdeletebutton(index)) {
			...
			actions.push({
				id: `${fliId}_delete_btn`,
				type: "click",
				method: files_delete(index),
			});
		}
		content += `<button id='${fliId}_rename_btn' ${btnCls} ${btnPad}>${get_icon_svg("wrench", "1em", "1em")}</button>`;
		actions.push({
			id: `${fliId}_rename_btn`,
			type: "click",
			method: files_rename(index),
		});
		content += "</div>";
		content += "</div>";
		content += "</div>";
