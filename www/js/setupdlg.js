...
function buildControlItem(label, pos, actions, extra) {
	return (
		...
		table(build_control_from_pos(pos, actions, extra))
	);
}
...
const setupdlg = () => {
	...
	id("setupDlgCancel").addEventListener("click", (event) =>
		closeModal("cancel"),
	);

	id("startsteplink").addEventListener("click", (event) =>
		openstep(event, "startstep"),
	);
	id("step1link").addEventListener("click", (event) =>
		openstep(event, "step1"),
	);
	id("step2link").addEventListener("click", (event) =>
		openstep(event, "step2"),
	);
	id("step3link").addEventListener("click", (event) =>
		openstep(event, "step3"),
	);
	id("endsteplink").addEventListener("click", (event) =>
		openstep(event, "endstep"),
	);

	id("wizard_button").addEventListener("click", (event) =>
		continue_setup_wizard(),
	);
	...
	const content = table(
		td(`${get_icon_svg("flag")}&nbsp;`) +
			td(build_language_list("language_selection")),
	);
	setHTML("setup_langage_list", content);
	id("language_selection").addEventListener("change", (event) =>
...
function enablestep3() {
	...
	content += buildControlItem(
		"Check update using direct SD access:",
		EP_DIRECT_SD_CHECK,
		actions,
	);
	content += spacer();
