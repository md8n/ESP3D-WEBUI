...
const restartdlg = () => {
	...
	setHTML("restartmsg", trans_text_item("Restarting, please wait...."));
	showModal();
...
function restart_esp_success(response) {
	...
	const interval = setInterval(() => {
		...
		const x = id("prgrestart");
		x.value = i;
		setHTML("restartmsg", `${trans_text_item("Restarting, please wait....")} ${x.max + 1 - i} ${trans_text_item("seconds")}`);
		if (i > x.max) {
...
function restart_esp_failed(error_code, response) {
	...
	setHTML("restartmsg", stdErrMsg(error_code, response, trans_text_item("Upload failed")));
