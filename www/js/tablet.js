...
function doPlayButton() {
	...
	text +=
		"\n" +
		"Starting File: " +
		document.getElementById("filelist").options[selectElement.selectedIndex]
			.text;
	msgWindow.textContent = text;
	msgWindow.scrollTop = msgWindow.scrollHeight;
}
...
const tabletInit = () => {
	...
	setTimeout(() => {
		...
		id("tabelttab_config_popup_content").addEventListener("click", (event) =>
			event.stopPropagation(),
		);

		id("tablettab_zUp").addEventListener("click", (event) => sendMove("Z+"));
		id("tablettab_topLeft").addEventListener("click", (event) =>
			sendMove("X-Y+"),
		);
		id("tablettab_top").addEventListener("click", (event) => sendMove("Y+"));
		id("tablettab_topRight").addEventListener("click", (event) =>
			sendMove("X+Y+"),
		);
		...
		id("tablettab_bottomLeft").addEventListener("click", (event) =>
			sendMove("X-Y-"),
		);
		id("tablettab_bottom").addEventListener("click", (event) => sendMove("Y-"));
		id("tablettab_bottomRight").addEventListener("click", (event) =>
			sendMove("X+Y-"),
		);

		id("tablettab_set_z_home").addEventListener("mousedown", (event) =>
			zeroAxis("Z"),
		);
		id("tablettab_set_z_home").addEventListener("mouseup", (event) =>
			refreshGcode(),
		);
		id("tablettab_move_to_xy_home").addEventListener("click", (event) =>
			moveHome(),
		);
		id("tablettab_toggle_units").addEventListener("click", (event) =>
			toggleUnits(),
		);
		id("tablettab_set_xy_home").addEventListener("mousedown", (event) =>
			setHomeClickDown(),
		);
		id("tablettab_set_xy_home").addEventListener("mouseup", (event) =>
			setHomeClickUp(),
		);
		id("tablettab_set_xy_home").addEventListener("dblclick", (event) =>
			setXYHome(),
		);

		id("tablettab_gcode_upload").addEventListener("click", (event) =>
			files_select_upload(),
		);
		id("tablettab_gcode_play").addEventListener("click", (event) =>
			doPlayButton(),
		);
		id("tablettab_gcode_pause").addEventListener("click", (event) =>
			doPauseButton(),
		);
		id("tablettab_gcode_stop").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$STOP", "Stop Maslow and Gcode"),
		);

		id("tablettab_cal_retract").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$ALL", "Retract All"),
		);
		id("tablettab_cal_extend").addEventListener("click", (event) =>
		...
		id("tablettab_cal_stop").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$STOP", "Stop"),
		);
		id("tablettab_cal_zstop").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$SETZSTOP", "Set Z-Stop"),
		);
		id("tablettab_cal_test").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$TEST", "Test"),
		);
		id("tablettab_cal_relax").addEventListener("click", (event) =>
			onCalibrationButtonsClick("$CMP", "Release Tension"),
		);
		id("tablettab_config_save").addEventListener("click", (event) =>
			saveConfigValues(),
		);

		id("tablettab_save_serial_msg").addEventListener("click", (event) =>
			saveSerialMessages(),
		);

		id("calibration-popup").addEventListener("click", (event) =>
			hideModal("calibration-popup"),
		);
		id("calibration_popup_content").addEventListener("click", (event) =>
			event.stopPropagation(),
		);
		id("configuration-popup").addEventListener("click", (event) =>
